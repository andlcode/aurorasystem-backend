...
async function getDashboard(req, res) {
    ...
    const classWhere = canViewAll || !userId ? {} : { responsibleUserId: userId };
...
async function listDiaryAttendanceByClasses(req, res) {
    const parsed = stats_dto_1.monthlyAttendanceQuerySchema.safeParse(req.query);
    if (!parsed.success) {
        res.status(400).json({ error: "Filtros inválidos", details: parsed.error.errors });
        return;
...
async function listMonthlyAttendanceByClasses(req, res) {
    const parsed = stats_dto_1.monthlyAttendanceQuerySchema.safeParse(req.query);
    if (!parsed.success) {
        res.status(400).json({ error: "Filtros inválidos", details: parsed.error.errors });
        return;
