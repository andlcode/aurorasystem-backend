...
async function createClass(data, createdByPersonId) {
    const responsible = await prisma_1.prisma.people.findUnique({
...
async function patchClass(classId, data, role, personId) {
    const existing = await prisma_1.prisma.class.findUnique({ where: { id: classId } });
    if (!existing)
        return null;
...
async function addParticipant(classId, data) {
    ...
    const existing = await prisma_1.prisma.classParticipant.findUnique({
        where: {
    ...
    const participantClasses = await prisma_1.prisma.classParticipant.findMany({
    ...
    return prisma_1.prisma.classParticipant.create({
        data: {
            classId,
...
async function removeParticipant(classId, participantId) {
    const cp = await prisma_1.prisma.classParticipant.findUnique({
        where: {
    ...
    await prisma_1.prisma.classParticipant.delete({
        where: {
            classId_participantId: { classId, participantId },
        },
    });
...
async function listParticipants(classId) {
    const participants = await prisma_1.prisma.classParticipant.findMany({
        where: { classId },
        include: { participant: true },
