 ...
 */
export function getLocalDateStringAmericaBahia(): string {
  return DateTime.now().setZone(TZ_BAHIA).toISODate() ?? "";
}
 ...
 */
export function getCurrentMonthRangeBahia(): { start: Date; end: Date } {
  const now = DateTime.now().setZone(TZ_BAHIA);
